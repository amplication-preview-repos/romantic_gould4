/*
------------------------------------------------------------------------------ 
This code was generated by Amplication. 
 
Changes to this file will be lost if the code is regenerated. 

There are other ways to to customize your code, see this doc to learn more
https://docs.amplication.com/how-to/custom-code

------------------------------------------------------------------------------
  */
import { InputType, Field } from "@nestjs/graphql";
import { ApiProperty } from "@nestjs/swagger";
import {
  IsString,
  MaxLength,
  IsOptional,
  ValidateNested,
  IsNumber,
  Min,
  Max,
} from "class-validator";
import { BuyUpdateManyWithoutTokensInput } from "./BuyUpdateManyWithoutTokensInput";
import { Type } from "class-transformer";
import { NotificationUpdateManyWithoutTokensInput } from "./NotificationUpdateManyWithoutTokensInput";
import { SellUpdateManyWithoutTokensInput } from "./SellUpdateManyWithoutTokensInput";

@InputType()
class TokenUpdateInput {
  @ApiProperty({
    required: false,
    type: String,
  })
  @IsString()
  @MaxLength(1000)
  @IsOptional()
  @Field(() => String, {
    nullable: true,
  })
  address?: string | null;

  @ApiProperty({
    required: false,
    type: () => BuyUpdateManyWithoutTokensInput,
  })
  @ValidateNested()
  @Type(() => BuyUpdateManyWithoutTokensInput)
  @IsOptional()
  @Field(() => BuyUpdateManyWithoutTokensInput, {
    nullable: true,
  })
  buys?: BuyUpdateManyWithoutTokensInput;

  @ApiProperty({
    required: false,
    type: String,
  })
  @IsString()
  @MaxLength(1000)
  @IsOptional()
  @Field(() => String, {
    nullable: true,
  })
  name?: string | null;

  @ApiProperty({
    required: false,
    type: () => NotificationUpdateManyWithoutTokensInput,
  })
  @ValidateNested()
  @Type(() => NotificationUpdateManyWithoutTokensInput)
  @IsOptional()
  @Field(() => NotificationUpdateManyWithoutTokensInput, {
    nullable: true,
  })
  notifications?: NotificationUpdateManyWithoutTokensInput;

  @ApiProperty({
    required: false,
    type: Number,
  })
  @IsNumber()
  @Min(-999999999)
  @Max(999999999)
  @IsOptional()
  @Field(() => Number, {
    nullable: true,
  })
  price?: number | null;

  @ApiProperty({
    required: false,
    type: () => SellUpdateManyWithoutTokensInput,
  })
  @ValidateNested()
  @Type(() => SellUpdateManyWithoutTokensInput)
  @IsOptional()
  @Field(() => SellUpdateManyWithoutTokensInput, {
    nullable: true,
  })
  sells?: SellUpdateManyWithoutTokensInput;

  @ApiProperty({
    required: false,
    type: String,
  })
  @IsString()
  @MaxLength(1000)
  @IsOptional()
  @Field(() => String, {
    nullable: true,
  })
  symbolField?: string | null;
}

export { TokenUpdateInput as TokenUpdateInput };
